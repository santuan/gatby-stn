...
const Card = ({ card }) => (
  <div
    className="relative top-0 flex w-full overflow-hidden duration-700 transform bg-indigo-900 rounded-md shadow-lg hover:-translate-y-2"
  ...
  >
    ...
    <Link
      to={`/blog/${card.slug}`}
      className="absolute inset-0 z-0 w-full opacity-20 md:opacity-30 "
    >
      <GatsbyImage
        title={card.title}
