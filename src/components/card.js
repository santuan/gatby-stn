...
const Card = ({ card }) => (
  <div
    className="relative top-0 flex w-full overflow-hidden duration-700 transform bg-indigo-900 rounded-md shadow-lg group hover:scale-105"
    style={{ minHeight: "230px" }}
  >
    <div className="relative z-10 w-full px-6 py-6 pb-4 text-left duration-700 bg-opacity-75 sm:from-indigo-900 bg-gradient-to-br hover:bg-indigo-700 hover:bg-opacity-80">
    ...
    <Link
      to={`/blog/${card.slug}`}
      className="absolute inset-0 z-0 w-full duration-1000 opacity-20 md:opacity-40 group-hover:scale-105 "
    >
      <GatsbyImage
        title={card.title}
